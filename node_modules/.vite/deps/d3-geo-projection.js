import { A as projection, G as rotation_default, K as centroid_default, P as interpolate_default, S as azimuthalEquidistantRaw, W as circle_default, X as range, Y as stream_default, Z as merge, _ as equirectangular_default, d as gnomonic_default, j as projectionMutator, o as orthographic_default, q as bounds_default, u as gnomonicRaw, w as azimuthalEqualAreaRaw } from "./src-23x6NTLs.js";
//#region node_modules/d3-geo-projection/src/math.js
var abs = Math.abs;
var atan = Math.atan;
...
//#endregion
//#region node_modules/d3-geo-projection/src/airy.js
function airyRaw(beta) {
	var tanBeta_2 = tan(beta / 2), b = 2 * log(cos(beta / 2)) / (tanBeta_2 * tanBeta_2);
	function forward(x, y) {
		var cosx = cos(x), cosy = cos(y), siny = sin(y), cosz = cosy * cosx, k = -((1 - cosz ? log((1 + cosz) / 2) / (1 - cosz) : -.5) + b / (1 + cosz));
	...
	forward.invert = function(x, y) {
		...
		do {
			...
			z -= delta = (2 / tanz_2 * lnsecz_2 - b * tanz_2 - r) / (-lnsecz_2 / (sinz_2 * sinz_2) + 1 - b / (2 * cosz_2 * cosz_2)) * (cosz_2 < 0 ? .7 : 1);
...
//#endregion
//#region node_modules/d3-geo-projection/src/aitoff.js
function aitoffRaw(x, y) {
	var cosy = cos(y), sincia = sinci(acos(cosy * cos(x /= 2)));
	return [2 * cosy * sin(x) * sincia, sin(y) * sincia];
}
aitoffRaw.invert = function(x, y) {
	if (x * x + 4 * y * y > pi * pi + 1e-6) return;
	var x1 = x, y1 = y, i = 25;
	do {
		var sinx = sin(x1), sinx_2 = sin(x1 / 2), cosx_2 = cos(x1 / 2), siny = sin(y1), cosy = cos(y1), sin_2y = sin(2 * y1), sin2y = siny * siny, cos2y = cosy * cosy, sin2x_2 = sinx_2 * sinx_2, c = 1 - cos2y * cosx_2 * cosx_2, e = c ? acos(cosy * cosx_2) * sqrt(f = 1 / c) : f = 0, f, fx = 2 * e * cosy * sinx_2 - x, fy = e * siny - y, dxdx = f * (cos2y * sin2x_2 + e * cosy * cosx_2 * sin2y), dxdy = f * (.5 * sinx * sin_2y - e * 2 * siny * sinx_2), dydx = f * .25 * (sin_2y * sinx_2 - e * siny * cos2y * sinx), dydy = f * (sin2y * cosx_2 + e * sin2x_2 * cosy), z = dxdy * dydx - dydy * dxdx;
		if (!z) break;
		var dx = (fy * dxdy - fx * dydy) / z, dy = (fx * dydx - fy * dxdx) / z;
		x1 -= dx, y1 -= dy;
	} while ((abs(dx) > 1e-6 || abs(dy) > 1e-6) && --i > 0);
	return [x1, y1];
};
...
//#endregion
//#region node_modules/d3-geo-projection/src/armadillo.js
function armadilloRaw(phi0) {
	...
	function forward(lambda, phi) {
		...
		return [(1 + cosPhi) * sin(lambda), (sPhi0 * phi > -atan2(cosLambda, tanPhi0) - .001 ? 0 : -sPhi0 * 10) + k + sin(phi) * cosPhi0 - (1 + cosPhi) * sinPhi0 * cosLambda];
	}
	forward.invert = function(x, y) {
		...
		} while ((abs(dLambda) > 1e-6 || abs(dPhi) > 1e-6) && --i > 0);
		return sPhi0 * phi > -atan2(cos(lambda), tanPhi0) - .001 ? [lambda * 2, phi] : null;
	};
	return forward;
}
function armadillo_default() {
	...
	p.stream = function(stream) {
		...
		rotateStream.sphere = function() {
			...
			if (phi0) while (sPhi0 * (lambda -= 3 * sPhi0 * precision) >= -180) sphereStream.point(lambda, sPhi0 * -atan2(cos(lambda * radians / 2), tanPhi0) * degrees);
...
//#endregion
//#region node_modules/d3-geo-projection/src/august.js
...
augustRaw.invert = function(x, y) {
	...
	return [sign(x) * 2 * atan2(sinh(xi) * cosEta, .25 - d), sign(y) * 2 * atan2(coshXi * sin(eta), .25 + d)];
};
function august_default() {
	return projection(augustRaw).scale(66.1603);
}
//#endregion
//#region node_modules/d3-geo-projection/src/baker.js
...
//#endregion
//#region node_modules/d3-geo-projection/src/berghaus.js
...
function berghaus_default() {
	var lobes = 5, m = projectionMutator(berghausRaw), p = m(lobes), projectionStream = p.stream, epsilon = .01, cr = -cos(epsilon * radians), sr = sin(epsilon * radians);
	p.lobes = function(_) {
	...
	return p.scale(87.8076).center([0, 17.1875]).clipAngle(179.999);
}
//#endregion
//#region node_modules/d3-geo-projection/src/hammer.js
...
//#endregion
//#region node_modules/d3-geo-projection/src/newton.js
...
function solve2d(f, MAX_ITERATIONS, eps) {
	...
	return function(x, y, a, b) {
		...
		for (var i = 0; i < MAX_ITERATIONS; i++) {
			...
			err2 = h;
			var ea = (a > 0 ? -1 : 1) * eps, eb = (b > 0 ? -1 : 1) * eps, pa = f(a + ea, b), pb = f(a, b + eb), dxa = (pa[0] - p[0]) / ea, dya = (pa[1] - p[1]) / ea, dxb = (pb[0] - p[0]) / eb, dyb = (pb[1] - p[1]) / eb, D = dyb * dxa - dya * dxb, l = (abs(D) < .5 ? .5 : 1) / D;
			da = (ty * dxb - tx * dyb) * l;
			db = (tx * dya - ty * dxa) * l;
			a += da;
			b += db;
...
//#endregion
//#region node_modules/d3-geo-projection/src/bertin.js
function bertin1953Raw() {
	var hammer = hammerRaw(1.68, 2), fu = 1.4, k = 12;
	function forward(lambda, phi) {
		if (lambda + phi < -fu) {
			var u = (lambda - phi + 1.6) * (lambda + phi + fu) / 8;
			lambda += u;
			phi -= .8 * u * sin(phi + pi / 2);
		}
		var r = hammer(lambda, phi);
		var d = (1 - cos(lambda * phi)) / k;
		if (r[1] < 0) r[0] *= 1 + d;
		if (r[1] > 0) r[1] *= 1 + d / 1.5 * r[0] * r[0];
		return r;
	}
...
function bertin_default() {
	return projection(bertin1953Raw()).rotate([-16.5, -42]).scale(176.57).center([7.93, .09]);
}
//#endregion
//#region node_modules/d3-geo-projection/src/mollweide.js
...
function mollweide_default$1() {
	return projection(mollweideRaw).scale(169.529);
}
//#endregion
//#region node_modules/d3-geo-projection/src/boggs.js
...
//#endregion
//#region node_modules/d3-geo-projection/src/parallel1.js
function parallel1_default(projectAt) {
	var phi0 = 0, m = projectionMutator(projectAt), p = m(phi0);
	p.parallel = function(_) {
...
//#endregion
//#region node_modules/d3-geo-projection/src/sinusoidal.js
...
function sinusoidal_default$1() {
	return projection(sinusoidalRaw).scale(152.63);
}
//#endregion
//#region node_modules/d3-geo-projection/src/bonne.js
...
//#endregion
//#region node_modules/d3-geo-projection/src/bottomley.js
...
function bottomley_default() {
	var sinPsi = .5, m = projectionMutator(bottomleyRaw), p = m(sinPsi);
...
//#endregion
//#region node_modules/d3-geo-projection/src/bromley.js
var bromleyRaw = mollweideBromleyRaw(1, 4 / pi, pi);
function bromley_default() {
	return projection(bromleyRaw).scale(152.63);
}
//#endregion
//#region node_modules/d3-geo-projection/src/chamberlin.js
function distance(dPhi, c1, s1, c2, s2, dLambda) {
	var cosdLambda = cos(dLambda), r;
	if (abs(dPhi) > 1 || abs(dLambda) > 1) r = acos(s1 * s2 + c1 * c2 * cosdLambda);
	else {
		var sindPhi = sin(dPhi / 2), sindLambda = sin(dLambda / 2);
		r = 2 * asin(sqrt(sindPhi * sindPhi + c1 * c2 * sindLambda * sindLambda));
	}
	return abs(r) > 1e-6 ? [r, atan2(c2 * sin(dLambda), c1 * s2 - s1 * c2 * cosdLambda)] : [0, 0];
}
...
function chamberlinRaw(p0, p1, p2) {
	...
	var beta0 = angle$1(points[0].v[0], points[2].v[0], points[1].v[0]), beta1 = angle$1(points[0].v[0], points[1].v[0], points[2].v[0]), beta2 = pi - beta0;
	points[2].point[1] = 0;
...
function chamberlin(p0, p1, p2) {
	...
	}), R = [-c[0], -c[1]], r = rotation_default(R), f = chamberlinRaw(pointRadians(r(p0)), pointRadians(r(p1)), pointRadians(r(p2)));
	f.invert = solve2d(f);
...
//#endregion
//#region node_modules/d3-geo-projection/src/collignon.js
...
//#endregion
//#region node_modules/d3-geo-projection/src/craig.js
...
//#endregion
//#region node_modules/d3-geo-projection/src/craster.js
var sqrt3 = sqrt(3);
...
//#endregion
//#region node_modules/d3-geo-projection/src/cylindricalEqualArea.js
...
//#endregion
//#region node_modules/d3-geo-projection/src/cylindricalStereographic.js
...
//#endregion
//#region node_modules/d3-geo-projection/src/eckert1.js
...
//#endregion
//#region node_modules/d3-geo-projection/src/eckert2.js
...
//#endregion
//#region node_modules/d3-geo-projection/src/eckert3.js
...
//#endregion
//#region node_modules/d3-geo-projection/src/eckert4.js
function eckert4Raw(lambda, phi) {
	var k = (2 + halfPi) * sin(phi);
	phi /= 2;
	for (var i = 0, delta = Infinity; i < 10 && abs(delta) > 1e-6; i++) {
		var cosPhi = cos(phi);
...
//#endregion
//#region node_modules/d3-geo-projection/src/eckert5.js
...
//#endregion
//#region node_modules/d3-geo-projection/src/eckert6.js
function eckert6Raw(lambda, phi) {
	var k = (1 + halfPi) * sin(phi);
	for (var i = 0, delta = Infinity; i < 10 && abs(delta) > 1e-6; i++) phi -= delta = (phi + sin(phi) - k) / (1 + cos(phi));
	k = sqrt(2 + pi);
...
//#endregion
//#region node_modules/d3-geo-projection/src/eisenlohr.js
...
eisenlohrRaw.invert = function(x, y) {
	...
	} while ((abs(deltaLambda) > 1e-6 || abs(deltaPhi) > 1e-6) && --i > 0);
	return abs(abs(phi) - halfPi) < 1e-6 ? [0, phi] : i && [lambda, phi];
};
function eisenlohr_default() {
	return projection(eisenlohrRaw).scale(62.5271);
}
//#endregion
//#region node_modules/d3-geo-projection/src/fahey.js
...
//#endregion
//#region node_modules/d3-geo-projection/src/foucaut.js
...
//#endregion
//#region node_modules/d3-geo-projection/src/foucautSinusoidal.js
function foucautSinusoidalRaw(alpha) {
	var beta = 1 - alpha, equatorial = raw(pi, 0)[0] - raw(-pi, 0)[0], ratio = sqrt(2 * (raw(0, halfPi)[1] - raw(0, -halfPi)[1]) / equatorial);
...
function foucautSinusoidal_default() {
	var alpha = .5, m = projectionMutator(foucautSinusoidalRaw), p = m(alpha);
	p.alpha = function(_) {
...
//#endregion
//#region node_modules/d3-geo-projection/src/gilbert.js
...
function gilbert_default(projectionType) {
	...
	var projection = projectionType(), equirectangular = equirectangular_default().scale(degrees).precision(0).clipAngle(null).translate([0, 0]);
...
//#endregion
//#region node_modules/d3-geo-projection/src/gingery.js
...
function gingery_default() {
	var n = 6, rho = 30 * radians, cRho = cos(rho), sRho = sin(rho), m = projectionMutator(gingeryRaw), p = m(rho, n), stream_ = p.stream, epsilon = .01, cr = -cos(epsilon * radians), sr = sin(epsilon * radians);
	p.radius = function(_) {
	...
	return p.rotate([90, -40]).scale(91.7095).clipAngle(179.999);
}
//#endregion
//#region node_modules/d3-geo-projection/src/ginzburgPolyconic.js
...
//#endregion
//#region node_modules/d3-geo-projection/src/ginzburg4.js
var ginzburg4Raw = ginzburgPolyconic_default(2.8284, -1.6988, .75432, -.18071, 1.76003, -.38914, .042555);
function ginzburg4_default() {
	return projection(ginzburg4Raw).scale(149.995);
}
//#endregion
//#region node_modules/d3-geo-projection/src/ginzburg5.js
var ginzburg5Raw = ginzburgPolyconic_default(2.583819, -.835827, .170354, -.038094, 1.543313, -.411435, .082742);
function ginzburg5_default() {
	return projection(ginzburg5Raw).scale(153.93);
}
//#endregion
//#region node_modules/d3-geo-projection/src/ginzburg6.js
var ginzburg6Raw = ginzburgPolyconic_default(5 / 6 * pi, -.62636, -.0344, 0, 1.3493, -.05524, 0, .045);
function ginzburg6_default() {
	return projection(ginzburg6Raw).scale(130.945);
}
//#endregion
//#region node_modules/d3-geo-projection/src/ginzburg8.js
function ginzburg8Raw(lambda, phi) {
	var lambda2 = lambda * lambda, phi2 = phi * phi;
	return [lambda * (1 - .162388 * phi2) * (.87 - 952426e-9 * lambda2 * lambda2), phi * (1 + phi2 / 12)];
}
ginzburg8Raw.invert = function(x, y) {
	...
	do {
		...
		lambda -= delta = (lambda * (.87 - 952426e-9 * lambda4) - x) / (.87 - .00476213 * lambda4);
	} while (abs(delta) > 1e-6 && --i > 0);
	return [lambda, phi];
};
...
//#endregion
//#region node_modules/d3-geo-projection/src/ginzburg9.js
var ginzburg9Raw = ginzburgPolyconic_default(2.6516, -.76534, .19123, -.047094, 1.36289, -.13965, .031762);
function ginzburg9_default() {
	return projection(ginzburg9Raw).scale(131.087);
}
//#endregion
//#region node_modules/d3-geo-projection/src/square.js
function square_default(project) {
	var dx = project(halfPi, 0)[0] - project(-halfPi, 0)[0];
	function projectSquare(lambda, phi) {
		var s = lambda > 0 ? -.5 : .5, point = project(lambda + s * pi, phi);
		point[0] -= s * dx;
		return point;
	}
	if (project.invert) projectSquare.invert = function(x, y) {
		var s = x > 0 ? -.5 : .5, location = project.invert(x + s * dx, y), lambda = location[0] - s * pi;
...
function gringortenHexadecant(lambda, phi) {
	...
	var cosPhi = cos(phi), secPhi = 1 / cosPhi, drdPhi = 2 * sinPhi * cosPhi, dvdPhi = (-3 * r + z * k) * drdPhi, dp2dPhi = (-v * cosPhi - (1 - sinPhi) * dvdPhi) / (v * v), dhdPhi = q * (.5 * dp2dPhi / p) - 2 * r * p * drdPhi, dra2dPhi = r * j * dp2dPhi + p2 * k * drdPhi, mu = -secPhi * drdPhi, nu = -secPhi * dra2dPhi, zeta = -2 * secPhi * dhdPhi, lambda1 = 4 * lambda / pi, delta;
	if (lambda > .222 * pi || phi < pi / 4 && lambda > .175 * pi) {
...
function gringortenHexadecantInvert(x, y) {
	...
	while (true) {
		var r2 = r * r, sinPhi = sqrt(r), z = asin(1 / sqrt(1 + r2)), v = 1 - r2 + r * (1 + r2) * z, p2 = (1 - sinPhi) / v, p = sqrt(p2), a2 = p2 * (1 + r2), h = p * (1 - r2), g = sqrt(a2 - x * x), y0 = y + h + r * g;
		if (abs(x1 - x0) < 1e-12 || --i === 0 || y0 === 0) break;
		if (y0 > 0) x0 = r;
		else x1 = r;
		r = .5 * (x0 + x1);
	}
	if (!i) return null;
	var phi = asin(sinPhi), cosPhi = cos(phi), secPhi = 1 / cosPhi, drdPhi = 2 * sinPhi * cosPhi, dvdPhi = (-3 * r + z * (1 + 3 * r2)) * drdPhi, dp2dPhi = (-v * cosPhi - (1 - sinPhi) * dvdPhi) / (v * v), dpdPhi = .5 * dp2dPhi / p, dhdPhi = (1 - r2) * dpdPhi - 2 * r * p * drdPhi, zeta = -2 * secPhi * dhdPhi, mu = -secPhi * drdPhi, nu = -secPhi * (r * (1 + r2) * dp2dPhi + p2 * (1 + 3 * r2) * drdPhi);
...
//#endregion
//#region node_modules/d3-geo-projection/src/elliptic.js
...
function ellipticFi(phi, psi, m) {
	var r = abs(phi), sinhPsi = sinh(abs(psi));
	if (r) {
...
//#endregion
//#region node_modules/d3-geo-projection/src/guyou.js
function guyouRaw(lambda, phi) {
	var k_ = (sqrt2 - 1) / (sqrt2 + 1), k = sqrt(1 - k_ * k_), K = ellipticF(halfPi, k * k), f = -1, r = exp(f * log(tan(pi / 4 + abs(phi) / 2))) / sqrt(k_), at = guyouComplexAtan(r * cos(f * lambda), r * sin(f * lambda)), t = ellipticFi(at[0], at[1], k * k);
	return [-t[1], (phi >= 0 ? 1 : -1) * (.5 * K - t[0])];
}
function guyouComplexAtan(x, y) {
	var x2 = x * x, y_1 = y + 1, t = 1 - x2 - y * y;
	return [.5 * ((x >= 0 ? halfPi : -halfPi) - atan2(t, 2 * x)), -.25 * log(t * t + 4 * x2) + .5 * log(y_1 * y_1 + x2)];
}
...
guyouRaw.invert = function(x, y) {
	var k_ = (sqrt2 - 1) / (sqrt2 + 1), k = sqrt(1 - k_ * k_), K = ellipticF(halfPi, k * k), f = -1, j = ellipticJi(.5 * K - y, -x, k * k), tn = guyouComplexDivide(j[0], j[1]);
	return [atan2(tn[1], tn[0]) / f, 2 * atan(exp(.5 / f * log(k_ * tn[0] * tn[0] + k_ * tn[1] * tn[1]))) - halfPi];
};
function guyou_default() {
	return projection(square_default(guyouRaw)).scale(151.496);
}
//#endregion
//#region node_modules/d3-geo-projection/src/hammerRetroazimuthal.js
function hammerRetroazimuthalRaw(phi0) {
	...
	function forward(lambda, phi) {
		...
		lambda = p[0], phi = p[1];
		var sinPhi = sin(phi), cosPhi = cos(phi), cosLambda = cos(lambda), z = acos(sinPhi0 * sinPhi + cosPhi0 * cosPhi * cosLambda), sinz = sin(z), K = abs(sinz) > 1e-6 ? z / sinz : 1;
		return [K * cosPhi0 * sin(lambda), (abs(lambda) > halfPi ? K : -K) * (sinPhi0 * cosPhi - cosPhi0 * sinPhi * cosLambda)];
	}
	forward.invert = function(x, y) {
...
function hammerRetroazimuthal_default() {
	var phi0 = 0, m = projectionMutator(hammerRetroazimuthalRaw), p = m(phi0), rotate_ = p.rotate, stream_ = p.stream, circle = circle_default();
	p.parallel = function(_) {
		if (!arguments.length) return phi0 * degrees;
		var r = p.rotate();
	...
	p.stream = function(stream) {
		...
		stream.sphere = function() {
			stream.polygonStart();
			var epsilon = .01, ring = circle.radius(90 - epsilon)().coordinates[0], n = ring.length - 1, i = -1, p;
			stream.lineStart();
	...
	return p.scale(79.4187).parallel(45).clipAngle(179.999);
}
//#endregion
//#region node_modules/d3-geo-projection/src/healpix.js
var healpixParallel = asin(1 - 1 / 3) * degrees, healpixLambert = cylindricalEqualAreaRaw(0);
function healpixRaw(H) {
	...
	forward.invert = function(x, y) {
		...
		if (y2 > y0) {
			...
			var point = collignonRaw.invert(x, .25 * (y2 - y0) * tau / dy1 + y1);
...
//#endregion
//#region node_modules/d3-geo-projection/src/hill.js
function hillRaw(K) {
	var L = 1 + K, Bt = asin(sin(1 / L)), A = 2 * sqrt(pi / (B = pi + 4 * Bt * L)), B, rho0 = .5 * A * (L + sqrt(K * (2 + K))), K2 = K * K, L2 = L * L;
	function forward(lambda, phi) {
		...
		if (t && t < 2) {
			...
			do {
				...
				theta -= delta = (theta - K2 * Bt - L * sinTheta + C * Bt_Bt1 - .5 * t * B) / (2 * L * sinTheta * Bt_Bt1);
			} while (abs(delta) > 1e-12 && --i > 0);
			rho = A * sqrt(C);
...
var sinuMollweideY = .0528035274542;
function sinuMollweideRaw(lambda, phi) {
	return phi > -.7109889596207567 ? (lambda = mollweideRaw(lambda, phi), lambda[1] += sinuMollweideY, lambda) : sinusoidalRaw(lambda, phi);
}
sinuMollweideRaw.invert = function(x, y) {
	return y > -.7109889596207567 ? mollweideRaw.invert(x, y - sinuMollweideY) : sinusoidalRaw.invert(x, y);
};
function sinuMollweide_default$1() {
	return projection(sinuMollweideRaw).rotate([-20, -55]).scale(164.263).center([0, -5.4036]);
}
//#endregion
//#region node_modules/d3-geo-projection/src/homolosine.js
function homolosineRaw(lambda, phi) {
	return abs(phi) > .7109889596207567 ? (lambda = mollweideRaw(lambda, phi), lambda[1] -= phi > 0 ? sinuMollweideY : -sinuMollweideY, lambda) : sinusoidalRaw(lambda, phi);
}
homolosineRaw.invert = function(x, y) {
	return abs(y) > .7109889596207567 ? mollweideRaw.invert(x, y + (y > 0 ? sinuMollweideY : -sinuMollweideY)) : sinusoidalRaw.invert(x, y);
};
function homolosine_default() {
	return projection(homolosineRaw).scale(152.63);
}
//#endregion
//#region node_modules/d3-geo-projection/src/hufnagel.js
...
function hufnagel_default() {
	var a = 1, b = 0, psiMax = 45 * radians, ratio = 2, mutate = projectionMutator(hufnagelRaw), projection = mutate(a, b, psiMax, ratio);
...
//#endregion
//#region node_modules/d3-geo-projection/src/integrate.js
function adsimp(f, a, b, fa, fm, fb, V0, tol, maxdepth, depth, state) {
	if (state.nanEncountered) return NaN;
	var h = b - a, f1 = f(a + h * .25), f2 = f(b - h * .25), sl, sr, s2, m, V1, V2, err;
	if (isNaN(f1)) {
...
function integrate(f, a, b, tol, maxdepth) {
	...
	var fm = f(.5 * (a + b));
	var fb = f(b);
	return adsimp(f, a, b, fa, fm, fb, (fa + 4 * fm + fb) * (b - a) / 6, tol, maxdepth, 1, state);
}
//#endregion
//#region node_modules/d3-geo-projection/src/hyperelliptical.js
function hyperellipticalRaw(alpha, k, gamma) {
	...
	var G = 1 / z(1), n = 1e3, m = 1.00000001 * G, approx = [];
...
function hyperelliptical_default() {
	var alpha = 0, k = 2.5, gamma = 1.183136, m = projectionMutator(hyperellipticalRaw), p = m(alpha, k, gamma);
	p.alpha = function(_) {
...
//#endregion
//#region node_modules/d3-geo-projection/src/interrupted/index.js
...
function interrupted_default(project, lobes, inverse) {
	var sphere, bounds;
	function forward(lambda, phi) {
	...
	if (inverse) forward.invert = inverse(forward);
	else if (project.invert) forward.invert = function(x, y) {
	...
	p.stream = function(stream) {
		...
		p.rotate(rotate);
		rotateStream.sphere = function() {
			stream_default(sphere, sphereStream);
		};
		return rotateStream;
	};
	p.lobes = function(_) {
...
//#endregion
//#region node_modules/d3-geo-projection/src/interrupted/boggs.js
var lobes$6 = [[[
	[-180, 0],
	[-100, 90],
	[-40, 0]
...
function boggs_default$1() {
	return interrupted_default(boggsRaw, lobes$6).scale(160.857);
}
//#endregion
//#region node_modules/d3-geo-projection/src/interrupted/homolosine.js
var lobes$5 = [[[
	[-180, 0],
	[-100, 90],
	[-40, 0]
...
function homolosine_default$1() {
	return interrupted_default(homolosineRaw, lobes$5).scale(152.63);
}
//#endregion
//#region node_modules/d3-geo-projection/src/interrupted/mollweide.js
var lobes$4 = [[[
	[-180, 0],
	[-100, 90],
	[-40, 0]
...
function mollweide_default() {
	return interrupted_default(mollweideRaw, lobes$4).scale(169.529);
}
//#endregion
//#region node_modules/d3-geo-projection/src/interrupted/mollweideHemispheres.js
var lobes$3 = [[[
	[-180, 0],
	[-90, 90],
	[0, 0]
...
function mollweideHemispheres_default() {
	return interrupted_default(mollweideRaw, lobes$3).scale(169.529).rotate([20, 0]);
}
//#endregion
//#region node_modules/d3-geo-projection/src/interrupted/sinuMollweide.js
var lobes$2 = [[[
	[-180, 35],
	[-30, 90],
	[0, 35]
...
function sinuMollweide_default() {
	return interrupted_default(sinuMollweideRaw, lobes$2, solve2d).rotate([-20, -55]).scale(164.263).center([0, -5.4036]);
}
//#endregion
//#region node_modules/d3-geo-projection/src/interrupted/sinusoidal.js
var lobes$1 = [[
	[
		[-180, 0],
		[-110, 90],
...
function sinusoidal_default() {
	return interrupted_default(sinusoidalRaw, lobes$1).scale(152.63).rotate([-20, 0]);
}
//#endregion
//#region node_modules/d3-geo-projection/src/kavrayskiy7.js
...
//#endregion
//#region node_modules/d3-geo-projection/src/lagrange.js
function lagrangeRaw(n) {
	function forward(lambda, phi) {
		if (abs(abs(phi) - halfPi) < 1e-6) return [0, phi < 0 ? -2 : 2];
		var sinPhi = sin(phi), v = pow((1 + sinPhi) / (1 - sinPhi), n / 2), c = .5 * (v + 1 / v) + cos(lambda *= n);
		return [2 * sin(lambda) / c, (v - 1 / v) / c];
	}
	forward.invert = function(x, y) {
		var y0 = abs(y);
		if (abs(y0 - 2) < 1e-6) return x ? null : [0, sign(y) * halfPi];
		if (y0 > 2) return null;
		x /= 2, y /= 2;
...
function lagrange_default() {
	var n = .5, m = projectionMutator(lagrangeRaw), p = m(n);
...
//#endregion
//#region node_modules/d3-geo-projection/src/larrivee.js
var pi_sqrt2 = pi / sqrt2;
function larriveeRaw(lambda, phi) {
	return [lambda * (1 + sqrt(cos(phi))) / 2, phi / (cos(phi / 2) * cos(lambda / 6))];
}
larriveeRaw.invert = function(x, y) {
	var x0 = abs(x), y0 = abs(y), lambda = epsilon$1, phi = halfPi;
	if (y0 < pi_sqrt2) phi *= y0 / pi_sqrt2;
	else lambda += 6 * acos(pi_sqrt2 / y0);
	for (var i = 0; i < 25; i++) {
		var sinPhi = sin(phi), sqrtcosPhi = sqrt(cos(phi)), sinPhi_2 = sin(phi / 2), cosPhi_2 = cos(phi / 2), sinLambda_6 = sin(lambda / 6), cosLambda_6 = cos(lambda / 6), f0 = .5 * lambda * (1 + sqrtcosPhi) - x0, f1 = phi / (cosPhi_2 * cosLambda_6) - y0, df0dPhi = sqrtcosPhi ? -.25 * lambda * sinPhi / sqrtcosPhi : 0, df0dLambda = .5 * (1 + sqrtcosPhi), df1dPhi = (1 + .5 * phi * sinPhi_2 / cosPhi_2) / (cosPhi_2 * cosLambda_6), df1dLambda = phi / cosPhi_2 * (sinLambda_6 / 6) / (cosLambda_6 * cosLambda_6), denom = df0dPhi * df1dLambda - df1dPhi * df0dLambda, dPhi = (f0 * df1dLambda - f1 * df0dLambda) / denom, dLambda = (f1 * df0dPhi - f0 * df1dPhi) / denom;
		phi -= dPhi;
		lambda -= dLambda;
		if (abs(dPhi) < 1e-6 && abs(dLambda) < 1e-6) break;
	}
...
//#endregion
//#region node_modules/d3-geo-projection/src/laskowski.js
function laskowskiRaw(lambda, phi) {
	var lambda2 = lambda * lambda, phi2 = phi * phi;
	return [lambda * (.975534 + phi2 * (-.119161 + lambda2 * -.0143059 + phi2 * -.0547009)), phi * (1.00384 + lambda2 * (.0802894 + phi2 * -.02855 + lambda2 * 199025e-9) + phi2 * (.0998909 + phi2 * -.0491032))];
}
laskowskiRaw.invert = function(x, y) {
	var lambda = sign(x) * pi, phi = y / 2, i = 50;
	do {
		var lambda2 = lambda * lambda, phi2 = phi * phi, lambdaPhi = lambda * phi, fx = lambda * (.975534 + phi2 * (-.119161 + lambda2 * -.0143059 + phi2 * -.0547009)) - x, fy = phi * (1.00384 + lambda2 * (.0802894 + phi2 * -.02855 + lambda2 * 199025e-9) + phi2 * (.0998909 + phi2 * -.0491032)) - y, deltaxDeltaLambda = .975534 - phi2 * (.119161 + 3 * lambda2 * .0143059 + phi2 * .0547009), deltaxDeltaPhi = -lambdaPhi * (2 * .119161 + 4 * .0547009 * phi2 + 2 * .0143059 * lambda2), deltayDeltaLambda = lambdaPhi * (2 * .0802894 + 4 * 199025e-9 * lambda2 + 2 * -.02855 * phi2), deltayDeltaPhi = 1.00384 + lambda2 * (.0802894 + 199025e-9 * lambda2) + phi2 * (3 * (.0998909 - .02855 * lambda2) - 5 * .0491032 * phi2), denominator = deltaxDeltaPhi * deltayDeltaLambda - deltayDeltaPhi * deltaxDeltaLambda, deltaLambda = (fy * deltaxDeltaPhi - fx * deltayDeltaPhi) / denominator, deltaPhi = (fx * deltayDeltaLambda - fy * deltaxDeltaLambda) / denominator;
...
//#endregion
//#region node_modules/d3-geo-projection/src/littrow.js
function littrowRaw(lambda, phi) {
	return [sin(lambda) / cos(phi), tan(phi) * cos(lambda)];
}
littrowRaw.invert = function(x, y) {
	var x2 = x * x, y2_1 = y * y + 1, x2_y2_1 = x2 + y2_1, cosPhi = x ? sqrt1_2 * sqrt((x2_y2_1 - sqrt(x2_y2_1 * x2_y2_1 - 4 * x2)) / x2) : 1 / sqrt(y2_1);
	return [asin(x * cosPhi), sign(y) * acos(cosPhi)];
};
function littrow_default() {
	return projection(littrowRaw).scale(144.049).clipAngle(89.999);
}
//#endregion
//#region node_modules/d3-geo-projection/src/loximuthal.js
function loximuthalRaw(phi0) {
	var cosPhi0 = cos(phi0), tanPhi0 = tan(quarterPi + phi0 / 2);
	function forward(lambda, phi) {
		var y = phi - phi0, x = abs(y) < 1e-6 ? lambda * cosPhi0 : abs(x = quarterPi + phi / 2) < 1e-6 || abs(abs(x) - halfPi) < 1e-6 ? 0 : lambda * y / log(tan(x) / tanPhi0);
		return [x, y];
	}
	forward.invert = function(x, y) {
		var lambda, phi = y + phi0;
		return [abs(y) < 1e-6 ? x / cosPhi0 : abs(lambda = quarterPi + phi / 2) < 1e-6 || abs(abs(lambda) - halfPi) < 1e-6 ? 0 : x * log(tan(lambda) / tanPhi0) / y, phi];
	};
	return forward;
}
...
//#endregion
//#region node_modules/d3-geo-projection/src/miller.js
function millerRaw(lambda, phi) {
	return [lambda, 1.25 * log(tan(quarterPi + .4 * phi))];
}
millerRaw.invert = function(x, y) {
	return [x, 2.5 * atan(exp(.8 * y)) - .625 * pi];
};
function miller_default() {
	return projection(millerRaw).scale(108.318);
}
//#endregion
//#region node_modules/d3-geo-projection/src/modifiedStereographic.js
function modifiedStereographicRaw(C) {
	...
	forward.invert = function(x, y) {
		...
		} while (abs(deltar) + abs(deltai) > 1e-6 * 1e-6 && --i > 0);
		if (i) {
			var rho = sqrt(zr * zr + zi * zi), c = 2 * atan(rho * .5), sinc = sin(c);
...
//#endregion
//#region node_modules/d3-geo-projection/src/mtFlatPolarParabolic.js
...
//#endregion
//#region node_modules/d3-geo-projection/src/mtFlatPolarQuartic.js
function mtFlatPolarQuarticRaw(lambda, phi) {
	var k = (1 + sqrt1_2) * sin(phi), theta = phi;
	for (var i = 0, delta; i < 25; i++) {
		theta -= delta = (sin(theta / 2) + sin(theta) - k) / (.5 * cos(theta / 2) + cos(theta));
...
//#endregion
//#region node_modules/d3-geo-projection/src/mtFlatPolarSinusoidal.js
function mtFlatPolarSinusoidalRaw(lambda, phi) {
	var A = sqrt(6 / (4 + pi)), k = (1 + pi / 4) * sin(phi), theta = phi / 2;
	for (var i = 0, delta; i < 25; i++) {
		theta -= delta = (theta / 2 + sin(theta) - k) / (.5 + cos(theta));
		if (abs(delta) < 1e-6) break;
	}
	return [A * (.5 + cos(theta)) * lambda / 1.5, A * theta];
}
mtFlatPolarSinusoidalRaw.invert = function(x, y) {
	var A = sqrt(6 / (4 + pi)), theta = y / A;
	if (abs(abs(theta) - halfPi) < 1e-6) theta = theta < 0 ? -halfPi : halfPi;
	return [1.5 * x / (A * (.5 + cos(theta))), asin((theta / 2 + sin(theta)) / (1 + pi / 4))];
};
function mtFlatPolarSinusoidal_default() {
	return projection(mtFlatPolarSinusoidalRaw).scale(166.518);
}
//#endregion
//#region node_modules/d3-geo-projection/src/naturalEarth2.js
function naturalEarth2Raw(lambda, phi) {
	var phi2 = phi * phi, phi4 = phi2 * phi2, phi6 = phi2 * phi4;
	return [lambda * (.84719 - .13063 * phi2 + phi6 * phi6 * (-.04515 + .05494 * phi2 - .02326 * phi4 + .00331 * phi6)), phi * (1.01183 + phi4 * phi4 * (-.02625 + .01926 * phi2 - .00396 * phi4))];
}
naturalEarth2Raw.invert = function(x, y) {
	...
	do {
		phi2 = phi * phi;
		phi4 = phi2 * phi2;
		phi -= delta = (phi * (1.01183 + phi4 * phi4 * (-.02625 + .01926 * phi2 - .00396 * phi4)) - y) / (1.01183 + phi4 * phi4 * (9 * -.02625 + 11 * .01926 * phi2 + 13 * -.00396 * phi4));
	} while (abs(delta) > 1e-12 && --i > 0);
	phi2 = phi * phi;
	phi4 = phi2 * phi2;
	phi6 = phi2 * phi4;
	return [x / (.84719 - .13063 * phi2 + phi6 * phi6 * (-.04515 + .05494 * phi2 - .02326 * phi4 + .00331 * phi6)), phi];
};
function naturalEarth2_default() {
	return projection(naturalEarth2Raw).scale(175.295);
}
//#endregion
//#region node_modules/d3-geo-projection/src/nellHammer.js
...
//#endregion
//#region node_modules/d3-geo-projection/src/interrupted/quarticAuthalic.js
var lobes = [[[
	[-180, 0],
	[-90, 90],
	[0, 0]
...
function quarticAuthalic_default() {
	return interrupted_default(hammerRaw(Infinity), lobes).rotate([20, 0]).scale(152.63);
}
//#endregion
//#region node_modules/d3-geo-projection/src/nicolosi.js
...
nicolosiRaw.invert = function(x, y) {
	x /= halfPi;
	y /= halfPi;
	var x2 = x * x, x2y2 = x2 + y * y, pi2 = pi * pi;
...
//#endregion
//#region node_modules/d3-geo-projection/src/patterson.js
...
//#endregion
//#region node_modules/d3-geo-projection/src/polyconic.js
function polyconicRaw(lambda, phi) {
	if (abs(phi) < 1e-6) return [lambda, 0];
	var tanPhi = tan(phi), k = lambda * sin(phi);
	return [sin(k) / tanPhi, phi + (1 - cos(k)) / tanPhi];
}
polyconicRaw.invert = function(x, y) {
	if (abs(y) < 1e-6) return [x, 0];
	var k = x * x + y * y, phi = y * .5, i = 10, delta;
	do {
...
//#endregion
//#region node_modules/d3-geo-projection/src/polyhedral/matrix.js
...
//#endregion
//#region node_modules/d3-geo-projection/src/polyhedral/index.js
function polyhedral_default(root, face) {
	...
	function forward(lambda, phi) {
		...
		if (t = node.transform) return [t[0] * point[0] + t[1] * point[1] + t[2], -(t[3] * point[0] + t[4] * point[1] + t[5])];
		point[1] = -point[1];
		return point;
	}
	...
	function faceInvert(node, coordinates) {
		...
		for (var i = 0, n = children && children.length; i < n; ++i) if (p = faceInvert(children[i], coordinates)) return p;
	}
...
function outline(stream, node, parent) {
	...
	for (var i = 0; i < n; ++i) {
		...
		if (Array.isArray(edge)) {
			if (!inside) {
				stream.point((point = interpolate_default(edge[0], c)(epsilon$1))[0], point[1]);
				inside = true;
			}
			stream.point((point = interpolate_default(edge[1], c)(epsilon$1))[0], point[1]);
		} else {
			inside = false;
...
//#endregion
//#region node_modules/d3-geo-projection/src/polyhedral/octahedron.js
var octahedron = [
	[0, 90],
	[-90, 0],
	[0, 0],
...
//#endregion
//#region node_modules/d3-geo-projection/src/polyhedral/butterfly.js
...
//#endregion
//#region node_modules/d3-geo-projection/src/polyhedral/collignon.js
var kx = 2 / sqrt(3);
...
function collignon_default$1(faceProjection) {
	faceProjection = faceProjection || function(face) {
		var c = centroid_default({
			type: "MultiPoint",
			coordinates: face
...
//#endregion
//#region node_modules/d3-geo-projection/src/polyhedral/waterman.js
function waterman_default(faceProjection) {
	faceProjection = faceProjection || function(face) {
		var c = face.length === 6 ? centroid_default({
			type: "MultiPoint",
			coordinates: face
		}) : face[0];
		return gnomonic_default().scale(1).translate([0, 0]).rotate([-c[0], -c[1]]);
	};
	var w5 = octahedron_default.map(function(face) {
		var xyz = face.map(cartesian), n = xyz.length, a = xyz[n - 1], b, hexagon = [];
		for (var i = 0; i < n; ++i) {
			b = xyz[i];
			hexagon.push(spherical([
				a[0] * .9486832980505138 + b[0] * .31622776601683794,
				a[1] * .9486832980505138 + b[1] * .31622776601683794,
				a[2] * .9486832980505138 + b[2] * .31622776601683794
			]), spherical([
				b[0] * .9486832980505138 + a[0] * .31622776601683794,
				b[1] * .9486832980505138 + a[1] * .31622776601683794,
				b[2] * .9486832980505138 + a[2] * .31622776601683794
			]));
			a = b;
		}
		return hexagon;
	...
	w5.forEach(function(hexagon, j) {
		var face = octahedron_default[j], n = face.length, normals = cornerNormals[j] = [];
		for (var i = 0; i < n; ++i) {
			w5.push([
				face[i],
				hexagon[(i * 2 + 2) % (2 * n)],
...
//#endregion
//#region node_modules/d3-geo-projection/src/noop.js
var noop_default = () => {};
//#endregion
//#region node_modules/d3-geo-projection/src/project/clockwise.js
...
//#endregion
//#region node_modules/d3-geo-projection/src/project/contains.js
...
//#endregion
//#region node_modules/d3-geo-projection/src/project/index.js
...
//#endregion
//#region node_modules/d3-geo-projection/src/quincuncial/index.js
function quincuncial_default(project) {
	...
	if (project.invert) projectQuincuncial.invert = function(x0, y0) {
		var x = (x0 + y0) * sqrt1_2, y = (y0 - x0) * sqrt1_2, t = abs(x) < .5 * dx && abs(y) < .5 * dx;
		if (!t) {
	...
	return projection(projectQuincuncial).rotate([
		-90,
		-90,
		45
	]).clipAngle(179.999);
...
//#endregion
//#region node_modules/d3-geo-projection/src/quincuncial/gringorten.js
function gringorten_default$1() {
	return quincuncial_default(gringortenRaw).scale(176.423);
}
//#endregion
//#region node_modules/d3-geo-projection/src/quincuncial/peirce.js
function peirce_default() {
	return quincuncial_default(guyouRaw).scale(111.48);
}
//#endregion
//#region node_modules/d3-geo-projection/src/quantize.js
function quantize_default(input, digits) {
	...
	function quantizePointsNoDuplicates(input) {
		...
		if (output.length === 1 && input.length > 1) output.push(quantizePoint(input[input.length - 1]));
		return output;
	}
...
//#endregion
//#region node_modules/d3-geo-projection/src/rectangularPolyconic.js
function rectangularPolyconicRaw(phi0) {
	...
	forward.invert = function(x, y) {
		if (abs(y += phi0) < 1e-6) return [sinPhi0 ? 2 * atan(sinPhi0 * x / 2) / sinPhi0 : x, 0];
		var k = x * x + y * y, phi = 0, i = 10, delta;
		do {
			var tanPhi = tan(phi), secPhi = 1 / cos(phi), j = k - 2 * y * phi + phi * phi;
			phi -= delta = (tanPhi * j + 2 * (phi - y)) / (2 + j * secPhi * secPhi + 2 * (phi - y) * tanPhi);
		} while (abs(delta) > 1e-6 && --i > 0);
		var E = x * (tanPhi = tan(phi)), A = tan(abs(y) < abs(phi + 1 / tanPhi) ? asin(E) * .5 : acos(E) * .5 + pi / 4) / sin(phi);
...
//#endregion
//#region node_modules/d3-geo-projection/src/robinson.js
var K = [
	[.9986, -.062],
	[1, 0],
	[.9986, .062],
...
robinsonRaw.invert = function(x, y) {
	...
	do {
		var ay = K[i0][1], by = K[i0 + 1][1], cy = K[min(19, i0 + 2)][1], u = cy - ay, v = cy - 2 * by + ay, t = 2 * (abs(y) - by) / u, c = v / u, di = t * (1 - c * t * (1 - 2 * c * t));
		if (di >= 0 || i0 === 1) {
			phi = (y >= 0 ? 5 : -5) * (di + i);
			var j = 50, delta;
			do {
...
//#endregion
//#region node_modules/d3-geo-projection/src/satellite.js
...
//#endregion
//#region node_modules/d3-geo-projection/src/stitch.js
var epsilon = 1e-4, epsilonInverse = 1e4, x0 = -180, x0e = x0 + epsilon, x1 = 180, x1e = x1 - epsilon, y0 = -90, y0e = y0 + epsilon, y1 = 90, y1e = y1 - epsilon;
	...
	for (i = 0; i < n; ++i) {
		...
		if (fragment) {
			...
			if (startFragment) {
				...
				if (startFragment === endFragment) fragment.polygon.push(fragment.ring);
				else {
					fragment.index = n++;
...
//#endregion
//#region node_modules/d3-geo-projection/src/times.js
function timesRaw(lambda, phi) {
	var t = tan(phi / 2), s = sin(quarterPi * t);
	return [lambda * (.74482 - .34588 * s * s), 1.70711 * t];
}
timesRaw.invert = function(x, y) {
	var t = y / 1.70711, s = sin(quarterPi * t);
	return [x / (.74482 - .34588 * s * s), 2 * atan(t)];
};
function times_default() {
	return projection(timesRaw).scale(146.153);
}
//#endregion
//#region node_modules/d3-geo-projection/src/twoPoint.js
function twoPoint_default(raw, p0, p1) {
	var i = interpolate_default(p0, p1), o = i(.5), a = rotation_default([-o[0], -o[1]])(p0), b = i.distance / 2, y = -asin(sin(a[1] * radians) / sin(b)), R = [
		-o[0],
		-o[1],
		-(a[0] > 0 ? pi - y : y) * degrees
	], p = projection(raw(b)).rotate(R), r = rotation_default(R), center = p.center;
	delete p.rotate;
	p.center = function(_) {
		return arguments.length ? center(r(_)) : r.invert(center());
	};
...
//#endregion
//#region node_modules/d3-geo-projection/src/twoPointAzimuthal.js
...
//#endregion
//#region node_modules/d3-geo-projection/src/twoPointEquidistant.js
function twoPointEquidistantRaw(z0) {
	if (!(z0 *= 2)) return azimuthalEquidistantRaw;
	var lambdaa = -z0 / 2, lambdab = -lambdaa, z02 = z0 * z0, tanLambda0 = tan(lambdab), S = .5 / sin(lambdab);
	function forward(lambda, phi) {
...
//#endregion
//#region node_modules/d3-geo-projection/src/vanDerGrinten.js
function vanDerGrintenRaw(lambda, phi) {
	if (abs(phi) < 1e-6) return [lambda, 0];
	var sinTheta = abs(phi / halfPi), theta = asin(sinTheta);
	if (abs(lambda) < 1e-6 || abs(abs(phi) - halfPi) < 1e-6) return [0, sign(phi) * pi * tan(theta / 2)];
...
vanDerGrintenRaw.invert = function(x, y) {
	if (abs(y) < 1e-6) return [x, 0];
	if (abs(x) < 1e-6) return [0, halfPi * sin(2 * atan(y / pi))];
...
//#endregion
//#region node_modules/d3-geo-projection/src/vanDerGrinten2.js
...
vanDerGrinten2Raw.invert = function(x, y) {
	...
	var x1 = abs(x / pi), A = (1 - x1 * x1 - (y /= pi) * y) / (2 * x1), B = sqrt(A * A + 1);
...
//#endregion
//#region node_modules/d3-geo-projection/src/vanDerGrinten3.js
function vanDerGrinten3Raw(lambda, phi) {
	if (abs(phi) < 1e-6) return [lambda, 0];
	var sinTheta = phi / halfPi, theta = asin(sinTheta);
	if (abs(lambda) < 1e-6 || abs(abs(phi) - halfPi) < 1e-6) return [0, pi * tan(theta / 2)];
...
//#endregion
//#region node_modules/d3-geo-projection/src/vanDerGrinten4.js
...
vanDerGrinten4Raw.invert = function(x, y) {
	...
	var sy = sign(y);
	y = abs(y) / pi;
	var x1 = sign(x) * x / halfPi, D = (x1 * x1 - 1 + 4 * y * y) / abs(x1), D2 = D * D, B = y * (2 - (y > .5 ? min(y, abs(x)) : 0)), r = x * x + y * y, i = 50;
	do {
...
//#endregion
//#region node_modules/d3-geo-projection/src/wagner.js
...
function wagnerRaw(poleline, parallels, inflation, ratio) {
	...
	var m2 = acos(vinflation * cos(phi1)) / phi1, m1 = sin(poleline) / sin(m2 * halfPi), n = parallels / pi, k = sqrt(vratio * sin(poleline / 2) / sin(parallels / 2));
	return wagnerFormula(k / sqrt(n * m1 * m2), 1 / (k * sqrt(n * m1 * m2)), m1, m2, n);
}
function wagner() {
	var poleline = 65 * radians, parallels = 60 * radians, inflation = 20, ratio = 200, mutate = projectionMutator(wagnerRaw), projection = mutate(poleline, parallels, inflation, ratio);
...
//#endregion
//#region node_modules/d3-geo-projection/src/wagner4.js
...
//#endregion
//#region node_modules/d3-geo-projection/src/wagner6.js
...
//#endregion
//#region node_modules/d3-geo-projection/src/wiechel.js
...
function wiechel_default() {
	return projection(wiechelRaw).rotate([
		0,
		-90,
		45
...
//#endregion
//#region node_modules/d3-geo-projection/src/winkel3.js
...
//#endregion
export { airy_default as geoAiry, airyRaw as geoAiryRaw, aitoff_default as geoAitoff, aitoffRaw as geoAitoffRaw, armadillo_default as geoArmadillo, armadilloRaw as geoArmadilloRaw, august_default as geoAugust, augustRaw as geoAugustRaw, baker_default as geoBaker, bakerRaw as geoBakerRaw, berghaus_default as geoBerghaus, berghausRaw as geoBerghausRaw, bertin_default as geoBertin1953, bertin1953Raw as geoBertin1953Raw, boggs_default as geoBoggs, boggsRaw as geoBoggsRaw, bonne_default as geoBonne, bonneRaw as geoBonneRaw, bottomley_default as geoBottomley, bottomleyRaw as geoBottomleyRaw, bromley_default as geoBromley, bromleyRaw as geoBromleyRaw, chamberlin as geoChamberlin, chamberlinAfrica as geoChamberlinAfrica, chamberlinRaw as geoChamberlinRaw, collignon_default as geoCollignon, collignonRaw as geoCollignonRaw, craig_default as geoCraig, craigRaw as geoCraigRaw, craster_default as geoCraster, crasterRaw as geoCrasterRaw, cylindricalEqualArea_default as geoCylindricalEqualArea, cylindricalEqualAreaRaw as geoCylindricalEqualAreaRaw, cylindricalStereographic_default as geoCylindricalStereographic, cylindricalStereographicRaw as geoCylindricalStereographicRaw, eckert1_default as geoEckert1, eckert1Raw as geoEckert1Raw, eckert2_default as geoEckert2, eckert2Raw as geoEckert2Raw, eckert3_default as geoEckert3, eckert3Raw as geoEckert3Raw, eckert4_default as geoEckert4, eckert4Raw as geoEckert4Raw, eckert5_default as geoEckert5, eckert5Raw as geoEckert5Raw, eckert6_default as geoEckert6, eckert6Raw as geoEckert6Raw, eisenlohr_default as geoEisenlohr, eisenlohrRaw as geoEisenlohrRaw, fahey_default as geoFahey, faheyRaw as geoFaheyRaw, foucaut_default as geoFoucaut, foucautRaw as geoFoucautRaw, foucautSinusoidal_default as geoFoucautSinusoidal, foucautSinusoidalRaw as geoFoucautSinusoidalRaw, gilbert_default as geoGilbert, gingery_default as geoGingery, gingeryRaw as geoGingeryRaw, ginzburg4_default as geoGinzburg4, ginzburg4Raw as geoGinzburg4Raw, ginzburg5_default as geoGinzburg5, ginzburg5Raw as geoGinzburg5Raw, ginzburg6_default as geoGinzburg6, ginzburg6Raw as geoGinzburg6Raw, ginzburg8_default as geoGinzburg8, ginzburg8Raw as geoGinzburg8Raw, ginzburg9_default as geoGinzburg9, ginzburg9Raw as geoGinzburg9Raw, gringorten_default as geoGringorten, gringorten_default$1 as geoGringortenQuincuncial, gringortenRaw as geoGringortenRaw, guyou_default as geoGuyou, guyouRaw as geoGuyouRaw, hammer_default as geoHammer, hammerRaw as geoHammerRaw, hammerRetroazimuthal_default as geoHammerRetroazimuthal, hammerRetroazimuthalRaw as geoHammerRetroazimuthalRaw, healpix_default as geoHealpix, healpixRaw as geoHealpixRaw, hill_default as geoHill, hillRaw as geoHillRaw, homolosine_default as geoHomolosine, homolosineRaw as geoHomolosineRaw, hufnagel_default as geoHufnagel, hufnagelRaw as geoHufnagelRaw, hyperelliptical_default as geoHyperelliptical, hyperellipticalRaw as geoHyperellipticalRaw, interrupted_default as geoInterrupt, boggs_default$1 as geoInterruptedBoggs, homolosine_default$1 as geoInterruptedHomolosine, mollweide_default as geoInterruptedMollweide, mollweideHemispheres_default as geoInterruptedMollweideHemispheres, quarticAuthalic_default as geoInterruptedQuarticAuthalic, sinuMollweide_default as geoInterruptedSinuMollweide, sinusoidal_default as geoInterruptedSinusoidal, kavrayskiy7_default as geoKavrayskiy7, kavrayskiy7Raw as geoKavrayskiy7Raw, lagrange_default as geoLagrange, lagrangeRaw as geoLagrangeRaw, larrivee_default as geoLarrivee, larriveeRaw as geoLarriveeRaw, laskowski_default as geoLaskowski, laskowskiRaw as geoLaskowskiRaw, littrow_default as geoLittrow, littrowRaw as geoLittrowRaw, loximuthal_default as geoLoximuthal, loximuthalRaw as geoLoximuthalRaw, miller_default as geoMiller, millerRaw as geoMillerRaw, modifiedStereographic as geoModifiedStereographic, modifiedStereographicAlaska as geoModifiedStereographicAlaska, modifiedStereographicGs48 as geoModifiedStereographicGs48, modifiedStereographicGs50 as geoModifiedStereographicGs50, modifiedStereographicLee as geoModifiedStereographicLee, modifiedStereographicMiller as geoModifiedStereographicMiller, modifiedStereographicRaw as geoModifiedStereographicRaw, mollweide_default$1 as geoMollweide, mollweideRaw as geoMollweideRaw, mtFlatPolarParabolic_default as geoMtFlatPolarParabolic, mtFlatPolarParabolicRaw as geoMtFlatPolarParabolicRaw, mtFlatPolarQuartic_default as geoMtFlatPolarQuartic, mtFlatPolarQuarticRaw as geoMtFlatPolarQuarticRaw, mtFlatPolarSinusoidal_default as geoMtFlatPolarSinusoidal, mtFlatPolarSinusoidalRaw as geoMtFlatPolarSinusoidalRaw, naturalEarth2_default as geoNaturalEarth2, naturalEarth2Raw as geoNaturalEarth2Raw, nellHammer_default as geoNellHammer, nellHammerRaw as geoNellHammerRaw, nicolosi_default as geoNicolosi, nicolosiRaw as geoNicolosiRaw, patterson_default as geoPatterson, pattersonRaw as geoPattersonRaw, peirce_default as geoPeirceQuincuncial, polyconic_default as geoPolyconic, polyconicRaw as geoPolyconicRaw, polyhedral_default as geoPolyhedral, butterfly_default as geoPolyhedralButterfly, collignon_default$1 as geoPolyhedralCollignon, waterman_default as geoPolyhedralWaterman, project_default as geoProject, quantize_default as geoQuantize, quincuncial_default as geoQuincuncial, rectangularPolyconic_default as geoRectangularPolyconic, rectangularPolyconicRaw as geoRectangularPolyconicRaw, robinson_default as geoRobinson, robinsonRaw as geoRobinsonRaw, satellite_default as geoSatellite, satelliteRaw as geoSatelliteRaw, sinuMollweide_default$1 as geoSinuMollweide, sinuMollweideRaw as geoSinuMollweideRaw, sinusoidal_default$1 as geoSinusoidal, sinusoidalRaw as geoSinusoidalRaw, stitch_default as geoStitch, times_default as geoTimes, timesRaw as geoTimesRaw, twoPointAzimuthal as geoTwoPointAzimuthal, twoPointAzimuthalRaw as geoTwoPointAzimuthalRaw, twoPointAzimuthalUsa as geoTwoPointAzimuthalUsa, twoPointEquidistant as geoTwoPointEquidistant, twoPointEquidistantRaw as geoTwoPointEquidistantRaw, twoPointEquidistantUsa as geoTwoPointEquidistantUsa, vanDerGrinten_default as geoVanDerGrinten, vanDerGrinten2_default as geoVanDerGrinten2, vanDerGrinten2Raw as geoVanDerGrinten2Raw, vanDerGrinten3_default as geoVanDerGrinten3, vanDerGrinten3Raw as geoVanDerGrinten3Raw, vanDerGrinten4_default as geoVanDerGrinten4, vanDerGrinten4Raw as geoVanDerGrinten4Raw, vanDerGrintenRaw as geoVanDerGrintenRaw, wagner as geoWagner, wagner4_default as geoWagner4, wagner4Raw as geoWagner4Raw, wagner6_default as geoWagner6, wagner6Raw as geoWagner6Raw, wagner7 as geoWagner7, wagnerRaw as geoWagnerRaw, wiechel_default as geoWiechel, wiechelRaw as geoWiechelRaw, winkel3_default as geoWinkel3, winkel3Raw as geoWinkel3Raw };
